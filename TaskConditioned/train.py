...
ida = IDA_config.IDA
lambda_mean = ida.lambda_mean
lambda_var = ida.lambda_var
k = ida.K
...
def main():
    ...
    if ida.is_ida == False:
        if ida.mode == 'visible':
            datacfg = 'data/kaist_visible.data'
            weightfile = 'weights/kaist_visible_detector.weights'
        else:
            datacfg = 'data/kaist_thermal.data'
    ...
    global model
    model = Darknet(cfgfile, use_cuda=use_cuda, k = k)
    ...
    try:
        ...
        for epoch in range(init_epoch+1, max_epochs+1):
            ...
            if condition:
                ...
                if ida.is_ida:
                    iterates,cur_loss = train_ida(iterates,train_sampler)
                else:
...
def train_ida(iterates,train_sampler):
    global output
    ...
    for batch_idx, [(data, target), [mean_mat, var_mat]] in enumerate(tqdm.tqdm(train_loader)):

        iterates += 1
        data, target = data.to(device), target.to(device)
        mean_mat, var_mat = mean_mat.to(device), var_mat.to(device)
        
        optimizer.zero_grad()

        [output, (pre_mean, pre_var)] = model(data, is_ida = True)
        ...
        #sum(org_loss).backward(retain_graph = True)
        

        loss_mean = torch.mean(torch.pow(pre_mean - mean_mat, 2))
        loss_var = torch.mean(torch.pow(pre_var - var_mat, 2))
        
        loss_ida = (lambda_mean * loss_mean + lambda_var * loss_var) * batch_size

        mean_lst.append(loss_mean)
        var_lst.append(loss_var)
        ida_lst.append(loss_ida)
        
        loss_all = sum(org_loss) + loss_ida
        ...
        del data, target
        gc.collect()
    print('loss_mean = {}'.format(sum(mean_lst) / len(mean_lst)))
    print('loss_var = {}'.format(sum(var_lst) / len(var_lst)))
    print('loss_ida = {}'.format(sum(ida_lst) / len(ida_lst)))
...
def PR_Valid(valid_sampler):
    ...
    if ida.is_ida:
        ...
        with torch.no_grad():
            for [(data, target, org_w, org_h), tem] in tqdm.tqdm(valid_loader):
                data = data.to(device)
                if condition:
                    output, cls_output = model(data)
                else:
