...
class listDataset(Dataset):
    ...
    def __init__(self, root, shape=None, shuffle=True, crop=False, jitter=0.3, hue=0.1, saturation=1.5, exposure=1.5,
       ...
       self.ida = IDA_config.IDA
       if self.ida.is_ida:
           self.meanvar_dic = np.load('meanvar_dic.npy', allow_pickle = True).item()
    ...
    def __getitem__(self, index):
        ...
        if self.ida.is_ida:
            ...
            if self.train:
                if self.condition:
                    return [(img, (label, set_label)), self.meanvar_dic[img_name]]
                else:
                    # print('end function get item')
                    return [(img, label), self.meanvar_dic[img_name]]
            else:
                try:   #train_val data
                    return [(img, label, org_w, org_h), self.meanvar_dic[img_name]]
                except: #test data
