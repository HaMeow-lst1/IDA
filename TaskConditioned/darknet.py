...
class HiNet(nn.Module):
    '''
    [batch, 1024, 16, 20] ---> [batch, k] and [batch, k]
    '''
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfgfile, use_cuda=True, k = 8):
        super(Darknet, self).__init__()
        
        self.hinet = HiNet(k = k)
    ...
    def forward(self, x, is_ida = False):
        ind = -2
        outputs = dict()
        out_boxes = dict()
        outno = 0
        ...
        if is_ida:
            if self.condition:
                return [(x if outno == 0 else out_boxes),outputs[len(outputs)-1], self.hinet(self.feature)]
            else:
                return [x if outno == 0 else out_boxes, self.hinet(self.feature)]
